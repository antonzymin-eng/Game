...
namespace game::military {
    ...
    MilitaryUnit::MilitaryUnit(UnitType unit_type) : type(unit_type) {
        ...
        switch (unit_type) {
            case UnitType::LEVIES:
                ...
                primary_class = game::population::SocialClass::SERFS;
                break;
            ...
            case UnitType::SPEARMEN:
                ...
                primary_class = game::population::SocialClass::FREE_PEASANTS;
                break;
            ...
            case UnitType::CROSSBOWMEN:
                ...
                primary_class = game::population::SocialClass::CRAFTSMEN;
                break;
            ...
            case UnitType::HEAVY_CAVALRY:
                ...
                primary_class = game::population::SocialClass::HIGH_NOBILITY;
                break;
    ...
    bool MilitaryComponent::CanRecruit(UnitType unit_type, uint32_t quantity) const {
        ...
        uint32_t current_recruitment = GetTotalGarrisonStrength();
        return (current_recruitment + quantity <= recruitment_capacity);
    }
    ...
    void MilitarySystem::Update(float delta_time) {
        ...
        if (m_accumulated_time >= m_update_interval) {
            // Update all armies - TODO: Implement proper entity iteration when ECS query API is available
        ...
        if (m_monthly_timer >= 30.0f) { // Simulate 30 seconds = 1 month
            // Update all military provinces - TODO: Implement proper entity iteration
    ...
    bool MilitarySystem::RecruitUnit(types::EntityID province_id, UnitType unit_type, uint32_t quantity) {
        ...
        auto* population_comp = entity_manager ? entity_manager->GetComponent<game::population::PopulationComponent>(entity_handle) : nullptr;
        if (!population_comp) {
    ...
    MilitaryComponent* MilitarySystem::GetMilitaryComponent(types::EntityID province_id) {
        ...
        ::core::ecs::EntityID entity_handle(static_cast<uint64_t>(province_id), 1);
        return entity_manager->GetComponent<MilitaryComponent>(entity_handle);
    }
    ...
    ArmyComponent* MilitarySystem::GetArmyComponent(types::EntityID army_id) {
        ...
        ::core::ecs::EntityID entity_handle(static_cast<uint64_t>(army_id), 1);
        return entity_manager->GetComponent<ArmyComponent>(entity_handle);
    }
    ...
    // ============================================================================
    // MilitarySystem ECS Implementation
    // ============================================================================
    ...
    void MilitarySystem::Initialize() {
        ...
        m_initialized = true;
        ::core::logging::LogInfo("MilitarySystem", "Military System initialization complete");
    }
    ...
    void MilitarySystem::CreateMilitaryComponents(game::types::EntityID entity_id) {
        if (!m_initialized) {
            ::core::logging::LogWarning("MilitarySystem", "System not initialized, cannot create components");
            return;
        }

        ::core::logging::LogInfo("MilitarySystem", 
            "Creating military components for entity " + std::to_string(static_cast<int>(entity_id)));
        ...
        // Create main military component
        auto military_component = entity_manager->AddComponent<MilitaryComponent>(entity_handle);
        if (military_component) {
            auto& config = game::config::GameConfig::Instance();
            military_component->recruitment_capacity = config.GetInt("military.recruitment_capacity", 500);
            military_component->training_facilities = config.GetFloat("military.training_facilities", 0.3f);
            military_component->supply_infrastructure = config.GetFloat("military.supply_infrastructure", 0.5f);
            military_component->military_budget = config.GetFloat("military.budget", 100.0f);
            
            // Initialize unit availability
            military_component->unit_type_available[UnitType::LEVIES] = true;
            military_component->unit_type_available[UnitType::SPEARMEN] = true;
            
            ::core::logging::LogInfo("MilitarySystem", "Created MilitaryComponent");
        }

        // Create fortification component
        auto fortification_component = entity_manager->AddComponent<FortificationComponent>(entity_handle);
        if (fortification_component) {
            fortification_component->walls_level = 1;
            fortification_component->structural_integrity = 1.0;
            fortification_component->garrison_capacity = 500;
            fortification_component->supply_storage_capacity = 1000.0;
            fortification_component->current_supply_storage = 1000.0;
            
            ::core::logging::LogInfo("MilitarySystem", "Created FortificationComponent");
        }

        // Create military events component
        auto events_component = entity_manager->AddComponent<MilitaryEventsComponent>(entity_handle);
        if (events_component) {
            events_component->event_frequency_modifier = 1.0f;
            events_component->military_reputation = 0.5;
            events_component->max_history_size = 100;
            
            ::core::logging::LogInfo("MilitarySystem", "Created MilitaryEventsComponent");
        }

        ::core::logging::LogInfo("MilitarySystem", "Successfully created all military components");
    }
    ...
    bool MilitarySystem::RecruitUnit(game::types::EntityID province_id, UnitType unit_type, uint32_t quantity) {
        ...
        if (!military_component) {
            ::core::logging::LogWarning("MilitarySystem", 
                "No MilitaryComponent found for province " + std::to_string(static_cast<int>(province_id)));
            return false;
        }

        // Create the unit
        MilitaryUnit new_unit = CreateUnitTemplate(unit_type);
        new_unit.current_strength = quantity;
        new_unit.max_strength = quantity;
        
        military_component->garrison_units.push_back(new_unit);
        military_component->recruitment_spending += new_unit.recruitment_cost * quantity;

        ::core::logging::LogInfo("MilitarySystem", 
            "Recruited " + std::to_string(quantity) + " units of type " + 
    ...
    uint32_t MilitarySystem::GetTotalMilitaryStrength(game::types::EntityID province_id) const {
        ...
        for (const auto& unit : military_component->garrison_units) {
            total_strength += unit.current_strength;
        }
    ...
    MilitaryUnit MilitarySystem::CreateUnitTemplate(UnitType unit_type) const {
        MilitaryUnit unit(unit_type);
        
        // Set basic properties based on unit type
        switch (unit_type) {
            case UnitType::LEVIES:
            ...
            default:
                ...
                unit.attack_strength = 10.0;
                unit.defense_strength = 8.0;
    ...
    // Serialization methods (basic implementation)
